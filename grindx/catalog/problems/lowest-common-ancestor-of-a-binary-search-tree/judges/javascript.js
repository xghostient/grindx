...
const tc = loadCases("lowest-common-ancestor-of-a-binary-search-tree");
          const cases = tc.cases;
          const total = cases.length;
        const fn = sol.lowestCommonAncestor;
for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  const root = listToTree(c.input[0]);
  const p = findNode(root, c.input[1]);
  const q = findNode(root, c.input[2]);
