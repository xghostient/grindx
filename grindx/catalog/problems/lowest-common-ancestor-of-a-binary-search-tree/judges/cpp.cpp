...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("lowest-common-ancestor-of-a-binary-search-tree");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    TreeNode* root = list_to_tree(c["input"][0].get_array());
    TreeNode* p = find_node(root, c["input"][1].get<int>());
    TreeNode* q = find_node(root, c["input"][2].get<int>());
    TreeNode* result = lowestCommonAncestor(root, p, q);
    int actual = result ? result->val : 0;
