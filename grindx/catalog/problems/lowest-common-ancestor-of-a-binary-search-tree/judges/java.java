...
static String intMatrixString(int[][] arr) {
    return Arrays.deepToString(arr);
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("lowest-common-ancestor-of-a-binary-search-tree");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    List<Object> input = (List<Object>) c.get("input");
    TreeNode root = listToTree((List<Object>) input.get(0));
    TreeNode p = findNode(root, Common.toInt(input.get(1)));
    TreeNode q = findNode(root, Common.toInt(input.get(2)));
    TreeNode result = new Solution().lowestCommonAncestor(root, p, q);
    int actual = result == null ? 0 : result.val;
