...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("lowest-common-ancestor-of-a-binary-search-tree")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        root = list_to_tree(case["input"][0])
        p = find_node(root, case["input"][1])
        q = find_node(root, case["input"][2])
        result = mod.lowestCommonAncestor(root, p, q)
        actual = None if result is None else result.val
