...
static std::string string_matrix_to_string(const std::vector<std::vector<std::string>>& matrix) {
    ...
    return out;
        ...
        int main() {
            auto tc = load_cases("accounts-merge");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        auto accounts = get_string_matrix(c["input"][0]);
        auto actual = normalize_accounts(accountsMerge(accounts));
        auto expected = get_string_matrix(c["expected"]);
        if (actual != expected) {
            report_wa(i, c["input"].to_string(), string_matrix_to_string(expected), string_matrix_to_string(actual), total, c["category"].is_string() ? c["category"].get_string() : "");
        }
