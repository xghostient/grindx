...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("binary-tree-right-side-view");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    auto actual = rightSideView(list_to_tree(c["input"][0].get_array()));
    std::string actual_str = grindx_vector_to_string(actual);
