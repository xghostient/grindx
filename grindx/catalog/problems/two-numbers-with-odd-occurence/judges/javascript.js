...
const tc = loadCases("two-numbers-with-odd-occurence");
...
for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  let actual = sol.twoOddOccurrences(c.input[0].slice());
  if (Array.isArray(actual)) actual = actual.slice().sort((a, b) => a - b);
