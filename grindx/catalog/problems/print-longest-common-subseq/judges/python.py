...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("print-longest-common-subseq")
    cases = tc["cases"]
    total = len(cases)
    ...
    for i, case in enumerate(cases):
        a, b = case["input"]
        actual = mod.printLCS(a, b)
        if not isinstance(actual, str):
            report_wa(i, case, actual, total)
        if len(actual) != case["expected"] or not is_subsequence(actual, a) or not is_subsequence(actual, b):
            report_wa(i, case, actual, total)
