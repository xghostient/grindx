...
static std::string int_matrix_to_string(const std::vector<std::vector<int>>& matrix) {
    ...
    return json::Value(rows).to_string();
        ...
        int main() {
            auto tc = load_cases("combination-sum-ii");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto candidates = cases[i]["input"][0].get<std::vector<int>>();
        int target = cases[i]["input"][1].get<int>();
        auto actual = normalize_int_matrix(combinationSum2(candidates, target));
