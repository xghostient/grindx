...
static List<List<Integer>> normalizeIntMatrix(int[][] raw) {
    ...
    return rows;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("combination-sum-ii");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    List<Object> input = (List<Object>) c.get("input");
    int[] candidates = toIntArray(input.get(0));
    int target = Common.toInt(input.get(1));
    List<List<Integer>> actual = normalizeIntMatrix(new Solution().combinationSum2(candidates, target));
