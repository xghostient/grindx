...
static int[][][] toIntTensor3(List<Object> raw) {
    ...
    return out;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("floyd-warshall");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) { Map<String, Object> c = (Map<String, Object>) cases.get(i); List<Object> in = (List<Object>) c.get("input"); int[][] matrix = Common.toIntMatrix((List<Object>) in.get(0)); int[][] actual = new Solution().floydWarshall(cloneMatrix(matrix)); int[][] expected = Common.toIntMatrix((List<Object>) c.get("expected")); if (!Arrays.deepEquals(actual, expected)) Common.reportWA(i, in, Arrays.deepToString(expected), Arrays.deepToString(actual), total, (String) c.getOrDefault("category", ""));
