...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("topological-sort-bfs-kahns-algo")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        v = case["input"][0]
        adj = clone_matrix(case["input"][1])
        actual = mod.topoSort(v, adj)
        if not valid_topological_order(actual, v, case["input"][1]):
