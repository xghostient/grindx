...
static bool valid_course_order(const std::vector<int>& order, int num_courses, const std::vector<std::vector<int>>& prerequisites) {
    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("topological-sort-bfs-kahns-algo");
        ...
        auto actual = topoSort(v, adj);
        if (!valid_topological_order(actual, v, adj)) {
            report_wa(i, c["input"].to_string(), c["expected"].to_string(), grindx_vector_to_string(actual), total, c["category"].is_string() ? c["category"].get_string() : "");
        }
