...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("search-in-a-binary-search-tree");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    TreeNode* actual_root = searchBST(list_to_tree(c["input"][0].get_array()), c["input"][1].get<int>());
    json::Value actual(tree_to_list(actual_root));
