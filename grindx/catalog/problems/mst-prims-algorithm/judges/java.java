...
static int[][][] toIntTensor3(List<Object> raw) {
    ...
    return out;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("mst-prims-algorithm");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) { Map<String, Object> c = (Map<String, Object>) cases.get(i); List<Object> in = (List<Object>) c.get("input"); int V = Common.toInt(in.get(0)); int[][][] adj = toIntTensor3((List<Object>) in.get(1)); int actual = new Solution().spanningTree(V, adj); int expected = Common.toInt(c.get("expected")); if (actual != expected) Common.reportWA(i, in, expected, actual, total, (String) c.getOrDefault("category", ""));
