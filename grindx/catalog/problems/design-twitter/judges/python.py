...
def run(solution_path: str) -> None:
    ...
    for i, case in enumerate(cases):
        ...
        for j, op in enumerate(case["operations"]):
            ...
            actual = None
            if op == "postTweet":
                getattr(obj, "postTweet")(*args)
                actual = None

            if op == "getNewsFeed":
                result = getattr(obj, "getNewsFeed")(*args)
                actual = list(result) if isinstance(result, (list, tuple)) else result

            if op == "follow":
                getattr(obj, "follow")(*args)
                actual = None

            if op == "unfollow":
                getattr(obj, "unfollow")(*args)
                actual = None
            if actual != expected:
