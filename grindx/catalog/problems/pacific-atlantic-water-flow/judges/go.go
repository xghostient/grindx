...
func graphSharesIdentity(original *Node, clone *Node) bool {
	...
	return false
        ...
        func main() {
	tc := LoadCases("pacific-atlantic-water-flow")
	total := len(tc.Cases)
	for i, c := range tc.Cases {
		var heights [][]int
		_ = json.Unmarshal(c.Input[0], &heights)
		actual := normalizePairs(pacificAtlantic(cloneMatrix(heights)))
		var expected [][]int
		_ = json.Unmarshal(c.Expected, &expected)
		if !Compare(actual, normalizePairs(expected), "exact") {
			ReportWA(i, heights, expected, actual, total, c.Category)
		}
