...
static bool graph_shares_identity(Node* original, Node* clone) {
    ...
    return false;
        ...
        int main() {
            auto tc = load_cases("pacific-atlantic-water-flow");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        std::vector<std::vector<int>> heights = c["input"][0].get<std::vector<std::vector<int>>>();
        auto actual = normalize_pairs(pacificAtlantic(heights));
