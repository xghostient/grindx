...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("pacific-atlantic-water-flow")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        heights = clone_matrix(case["input"][0])
        actual = normalize_pairs(mod.pacificAtlantic(heights) or [])
        expected = normalize_pairs(case["expected"])
        if actual != expected:
