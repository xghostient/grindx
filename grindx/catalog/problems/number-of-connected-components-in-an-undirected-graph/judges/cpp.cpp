...
static bool valid_course_order(const std::vector<int>& order, int num_courses, const std::vector<std::vector<int>>& prerequisites) {
    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("number-of-connected-components-in-an-undirected-graph");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        int n = c["input"][0].get<int>();
        std::vector<std::vector<int>> edges = c["input"][1].get<std::vector<std::vector<int>>>();
        int actual = countComponents(n, edges);
