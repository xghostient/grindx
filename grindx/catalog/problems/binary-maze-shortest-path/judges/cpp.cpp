...
static std::vector<std::vector<std::vector<int>>> get_int_tensor3(const json::Value& value) {
    ...
    return out;
        ...
        int main() {
            auto tc = load_cases("binary-maze-shortest-path");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) { auto& c = cases[i]; auto grid = c["input"][0].get<std::vector<std::vector<int>>>(); int actual = shortestPathBinaryMatrix(grid); int expected = c["expected"].get<int>(); if (actual != expected) report_wa(i, c["input"].to_string(), std::to_string(expected), std::to_string(actual), total, c["category"].is_string() ? c["category"].get_string() : "");
