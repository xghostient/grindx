"""Generated judge for largest-subarr-with-zero-sum."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "maxLen", None)
    if fn is None:
        print("Missing function: maxLen", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("largest-subarr-with-zero-sum")
    total = len(tc["cases"])
