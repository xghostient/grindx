...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("convert-sorted-array-to-binary-search-tree");
    ...
    json::Value actual(tree_to_list(root));
    if (grindx_vector_to_string(inorder_values(root)) != grindx_vector_to_string(nums) || !is_balanced_tree(root)) {
