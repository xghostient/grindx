...
const tc = loadCases("convert-sorted-array-to-binary-search-tree");
          const cases = tc.cases;
          const total = cases.length;
        const fn = sol.sortedArrayToBST;
for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  const nums = [...c.input[0]];
  const root = fn(nums);
  const actualShape = treeToList(root);
  if (JSON.stringify(inorderValues(root)) !== JSON.stringify(nums) || !isBalancedTree(root)) reportWa(i, c, actualShape, total);
