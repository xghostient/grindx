...
func isBalancedTree(root *TreeNode) bool {
	...
	return ok
        ...
        func main() {
        	tc := LoadCases("convert-sorted-array-to-binary-search-tree")
        	total := len(tc.Cases)
        for idx, c := range tc.Cases {
	var nums []int
	json.Unmarshal(c.Input[0], &nums)
	root := sortedArrayToBST(append([]int{}, nums...))
	actualShape := TreeToList(root)
	if !Compare(inorderValues(root), nums, "exact") || !isBalancedTree(root) {
		ReportWA(idx, c.Input, nums, actualShape, total, c.Category)
	}
