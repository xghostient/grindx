...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("convert-sorted-array-to-binary-search-tree")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        nums = list(case["input"][0])
        root = mod.sortedArrayToBST(list(nums))
        actual_inorder = inorder_values(root)
        actual_shape = tree_to_list(root)
        if actual_inorder != nums or not is_balanced_tree(root):
            report_wa(i, case, actual_shape, total)
