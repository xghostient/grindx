...
static bool validate_alien_order(const std::vector<std::string>& words, const std::string& order) {
    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("alien-dictionary");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        auto words = c["input"][0].get<std::vector<std::string>>();
        std::string actual = alienOrder(words);
        std::string expected = c["expected"].get<std::string>();
        bool ok = expected.empty() ? actual.empty() : validate_alien_order(words, actual);
        if (!ok) {
