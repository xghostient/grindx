...
static boolean validateAlienOrder(String[] words, String order) {
    ...
    return true;
        ...
        public static void main(String[] args) {
            Map<String, Object> tc = Common.loadCases("alien-dictionary");
            List<Object> cases = (List<Object>) tc.get("cases");
            int total = cases.size();
            for (int i = 0; i < cases.size(); i++) {
        Map<String, Object> c = (Map<String, Object>) cases.get(i);
        List<Object> input = (List<Object>) c.get("input");
        String[] words = toStringArray((List<Object>) input.get(0));
        String actual = new Solution().alienOrder(Arrays.copyOf(words, words.length));
        String expected = String.valueOf(c.get("expected"));
        boolean ok = expected.isEmpty() ? actual.isEmpty() : validateAlienOrder(words, actual);
        if (!ok) {
