    ...
    int main() {
        auto tc = load_cases("max-combination-sum");
        auto& cases = tc["cases"].get_array();
        int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto a = cases[i]["input"][0].get<std::vector<int>>();
    auto b = cases[i]["input"][1].get<std::vector<int>>();
    int k = cases[i]["input"][2].get<int>();
    auto actual = maxSumCombinations(a, b, k);
