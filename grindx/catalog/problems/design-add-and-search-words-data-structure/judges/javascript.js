...
const Cls = result.WordDictionary;
if (typeof Cls !== "function") {
  process.stderr.write("Solution does not define a WordDictionary class\n");
  process.exit(2);
}
const tc = loadCases("design-add-and-search-words-data-structure");
...
for (let i = 0; i < cases.length; i++) {
  ...
  for (let j = 0; j < c.operations.length; j++) {
    ...
    let actual = null;
        if (op === "addWord") {
          obj.addWord(...args); actual = null;
        }
        if (op === "search") {
          actual = obj.search(...args);
        }
