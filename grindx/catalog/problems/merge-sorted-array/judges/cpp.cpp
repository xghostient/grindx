...
static std::string int_matrix_to_string(const std::vector<std::vector<int>>& matrix) {
    ...
    return json::Value(rows).to_string();
        ...
        int main() {
            auto tc = load_cases("merge-sorted-array");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto nums1 = cases[i]["input"][0].get<std::vector<int>>();
        int m = cases[i]["input"][1].get<int>();
        auto nums2 = cases[i]["input"][2].get<std::vector<int>>();
        int n = cases[i]["input"][3].get<int>();
        merge(nums1, m, nums2, n);
        auto expected = cases[i]["expected"].get<std::vector<int>>();
        if (nums1 != expected) report_wa(i, cases[i]["input"].to_string(), json::Value(expected).to_string(), json::Value(nums1).to_string(), total, cases[i]["category"].is_string() ? cases[i]["category"].get_string() : "");
