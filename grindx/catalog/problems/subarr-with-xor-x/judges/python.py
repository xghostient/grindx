"""Generated judge for subarr-with-xor-x."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "countSubarraysXOR", None)
    if fn is None:
        print("Missing function: countSubarraysXOR", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("subarr-with-xor-x")
    total = len(tc["cases"])
