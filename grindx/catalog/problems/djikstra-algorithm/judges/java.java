...
static int[][][] toIntTensor3(List<Object> raw) {
    ...
    return out;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("djikstra-algorithm");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) { Map<String, Object> c = (Map<String, Object>) cases.get(i); List<Object> in = (List<Object>) c.get("input"); int V = Common.toInt(in.get(0)); int[][][] adj = toIntTensor3((List<Object>) in.get(1)); int src = Common.toInt(in.get(2)); int[] actual = new Solution().dijkstra(V, adj, src); int[] expected = Common.toIntArray((List<Object>) c.get("expected")); if (!Arrays.equals(actual, expected)) Common.reportWA(i, in, Arrays.toString(expected), Arrays.toString(actual), total, (String) c.getOrDefault("category", ""));
