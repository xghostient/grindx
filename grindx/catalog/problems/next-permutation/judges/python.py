"""Generated judge for next-permutation."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "nextPermutation", None)
    if fn is None:
        print("Missing function: nextPermutation", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("next-permutation")
    total = len(tc["cases"])
