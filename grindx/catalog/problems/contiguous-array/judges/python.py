"""Generated judge for contiguous-array."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "findMaxLength", None)
    if fn is None:
        print("Missing function: findMaxLength", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("contiguous-array")
    total = len(tc["cases"])
