...
static List<String> gridToStringRows(char[][] board) {
    ...
    return result;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("flood-fill");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    List<Object> input = (List<Object>) c.get("input");
    int[][] image = Common.toIntMatrix((List<Object>) input.get(0));
    int sr = Common.toInt(input.get(1));
    int sc = Common.toInt(input.get(2));
    int color = Common.toInt(input.get(3));
    int[][] actual = new Solution().floodFill(cloneMatrix(image), sr, sc, color);
