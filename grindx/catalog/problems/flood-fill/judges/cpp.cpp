...
static bool graph_shares_identity(Node* original, Node* clone) {
    ...
    return false;
        ...
        int main() {
            auto tc = load_cases("flood-fill");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        std::vector<std::vector<int>> image = c["input"][0].get<std::vector<std::vector<int>>>();
        int sr = c["input"][1].get<int>();
        int sc = c["input"][2].get<int>();
        int color = c["input"][3].get<int>();
        auto actual = floodFill(image, sr, sc, color);
