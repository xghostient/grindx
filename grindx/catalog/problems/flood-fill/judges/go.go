...
func graphSharesIdentity(original *Node, clone *Node) bool {
	...
	return false
        ...
        func main() {
	tc := LoadCases("flood-fill")
	total := len(tc.Cases)
	for i, c := range tc.Cases {
		var image [][]int
		_ = json.Unmarshal(c.Input[0], &image)
		var sr, sc, color int
		_ = json.Unmarshal(c.Input[1], &sr)
		_ = json.Unmarshal(c.Input[2], &sc)
		_ = json.Unmarshal(c.Input[3], &color)
		actual := floodFill(cloneMatrix(image), sr, sc, color)
		var expected [][]int
		_ = json.Unmarshal(c.Expected, &expected)
		if !Compare(actual, expected, "exact") {
			ReportWA(i, []any{image, sr, sc, color}, expected, actual, total, c.Category)
		}
