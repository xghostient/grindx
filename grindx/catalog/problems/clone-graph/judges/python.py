...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("clone-graph")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        graph = adj_list_to_graph([list(row) for row in case["input"][0]])
        cloned = mod.cloneGraph(graph)
        if graph_shares_identity(graph, cloned):
            report_wa(i, case, "shared original nodes", total)
        actual = graph_to_adj_list(cloned)
