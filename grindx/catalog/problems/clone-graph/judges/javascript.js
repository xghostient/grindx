...
function graphSharesIdentity(original, clone) {
  ...
  return false;
        ...
        function run(solutionPath) {
          ...
          const tc = loadCases("clone-graph");
          const cases = tc.cases;
          const total = cases.length;
        const fn = sol.cloneGraph;
for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  const original = adjListToGraph(c.input[0].map((row) => row.slice()));
  const cloned = fn(original);
  if (graphSharesIdentity(original, cloned)) reportWa(i, c, "shared original nodes", total);
  const actual = graphToAdjList(cloned);
