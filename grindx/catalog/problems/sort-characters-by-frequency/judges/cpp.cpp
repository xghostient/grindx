...
static bool is_valid_frequency_sort(const std::string& source, const std::string& actual) {
    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("sort-characters-by-frequency");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    std::string s = c["input"][0].get_string();
    std::string actual = frequencySort(s);
    if (!is_valid_frequency_sort(s, actual)) {
        report_wa(i, c["input"].to_string(), "valid frequency-sorted string", actual, total, c["category"].is_string() ? c["category"].get_string() : "");
    }
