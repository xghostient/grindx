"""Generated judge for maximum-product-subarray."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "maxProduct", None)
    if fn is None:
        print("Missing function: maxProduct", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("maximum-product-subarray")
    total = len(tc["cases"])
