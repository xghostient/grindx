...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("word-ladder-ii")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        actual = normalize_paths(mod.findLadders(case["input"][0], case["input"][1], list(case["input"][2])) or [])
        if actual != normalize_paths(case["expected"]):
