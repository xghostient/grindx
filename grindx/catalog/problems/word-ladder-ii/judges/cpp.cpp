...
static bool validate_alien_order(const std::vector<std::string>& words, const std::string& order) {
    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("word-ladder-ii");
        ...
        auto actual = normalize_paths(findLadders(beginWord, endWord, wordList));
        auto expected = normalize_paths(get_string_matrix(c["expected"]));
        if (actual != expected) {
