...
function validateAlienOrder(words, order) {
  ...
  return true;
        ...
        function run(solutionPath) {
          ...
          const tc = loadCases("word-ladder-ii");
          const cases = tc.cases;
          const total = cases.length;
        const fn = sol.findLadders;
for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  const actual = normalizePaths(fn(c.input[0], c.input[1], c.input[2].slice()) || []);
  const expected = normalizePaths(c.expected);
