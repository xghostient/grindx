...
function isValidFrequencySort(source, actual) {
  ...
  return true;
        ...
        const tc = loadCases("longest-palindromic-substring");
        const cases = tc.cases;
        const total = cases.length;

        for (let i = 0; i < cases.length; i++) {
  const testCase = cases[i];
  const actual = sol.longestPalindrome(testCase.input[0]);
  if (!isValidLongestPalindrome(testCase.input[0], testCase.expected, actual)) reportWa(i, testCase, actual, total);
