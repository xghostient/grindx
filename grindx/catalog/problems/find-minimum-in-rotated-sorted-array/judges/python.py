"""Generated judge for find-minimum-in-rotated-sorted-array."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "findMin", None)
    if fn is None:
        print("Missing function: findMin", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("find-minimum-in-rotated-sorted-array")
    total = len(tc["cases"])
