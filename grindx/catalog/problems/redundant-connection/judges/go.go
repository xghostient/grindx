...
func validCourseOrder(order []int, numCourses int, prerequisites [][]int) bool {
	...
	return true
        ...
        func main() {
	tc := LoadCases("redundant-connection")
	total := len(tc.Cases)
	for i, c := range tc.Cases {
		var edges [][]int
		_ = json.Unmarshal(c.Input[0], &edges)
		actual := findRedundantConnection(cloneMatrix(edges))
		var expected []int
		_ = json.Unmarshal(c.Expected, &expected)
		if !Compare(actual, expected, "exact") {
			ReportWA(i, edges, expected, actual, total, c.Category)
		}
