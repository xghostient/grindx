    ...
    int main() {
        auto tc = load_cases("lcm-gcd");
        auto& cases = tc["cases"].get_array();
        int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    int a = cases[i]["input"][0].get<int>();
    int b = cases[i]["input"][1].get<int>();
    auto actual = lcmAndGcd(a, b);
    auto expected = cases[i]["expected"].get<std::vector<long long>>();
