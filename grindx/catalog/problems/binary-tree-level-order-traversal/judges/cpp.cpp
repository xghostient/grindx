...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("binary-tree-level-order-traversal");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    auto actual = levelOrder(list_to_tree(c["input"][0].get_array()));
    std::string actual_str = grindx_matrix_to_string(actual);
    json::Value expected(c["expected"].get_array());
    if (actual_str != expected.to_string()) {
        std::string cat = c["category"].is_string() ? c["category"].get_string() : "";
        report_wa(i, c["input"].to_string(), expected.to_string(), actual_str, total, cat);
    }
