...
static String intMatrixString(int[][] arr) {
    return Arrays.deepToString(arr);
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("binary-tree-level-order-traversal");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    int[][] actual = new Solution().levelOrder(listToTree((List<Object>) ((List<Object>) c.get("input")).get(0)));
    String actualStr = intMatrixString(actual);
    String expectedStr = c.get("expected").toString();
    if (!actualStr.equals(expectedStr.replace(" ", "")) && !actualStr.replace(" ", "").equals(expectedStr.replace(" ", ""))) {
        Common.reportWA(i, c.get("input"), expectedStr, actualStr, total, (String) c.getOrDefault("category", ""));
    }
