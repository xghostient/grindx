...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("cheapest-flights-within-k-stops")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        actual = mod.findCheapestPrice(case["input"][0], clone_matrix(case["input"][1]), case["input"][2], case["input"][3], case["input"][4])
