...
static int[][][] toIntTensor3(List<Object> raw) {
    ...
    return out;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("cheapest-flights-within-k-stops");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) { Map<String, Object> c = (Map<String, Object>) cases.get(i); List<Object> in = (List<Object>) c.get("input"); int n = Common.toInt(in.get(0)); int[][] flights = Common.toIntMatrix((List<Object>) in.get(1)); int src = Common.toInt(in.get(2)); int dst = Common.toInt(in.get(3)); int k = Common.toInt(in.get(4)); int actual = new Solution().findCheapestPrice(n, flights, src, dst, k); int expected = Common.toInt(c.get("expected")); if (actual != expected) Common.reportWA(i, in, expected, actual, total, (String) c.getOrDefault("category", ""));
