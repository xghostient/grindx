...
static std::vector<std::vector<std::vector<int>>> get_int_tensor3(const json::Value& value) {
    ...
    return out;
        ...
        int main() {
            auto tc = load_cases("cheapest-flights-within-k-stops");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) { auto& c = cases[i]; int n = c["input"][0].get<int>(); auto flights = c["input"][1].get<std::vector<std::vector<int>>>(); int src = c["input"][2].get<int>(); int dst = c["input"][3].get<int>(); int k = c["input"][4].get<int>(); int actual = findCheapestPrice(n, flights, src, dst, k); int expected = c["expected"].get<int>(); if (actual != expected) report_wa(i, c["input"].to_string(), std::to_string(expected), std::to_string(actual), total, c["category"].is_string() ? c["category"].get_string() : "");
