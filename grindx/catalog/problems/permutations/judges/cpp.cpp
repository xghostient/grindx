        ...
        static std::vector<std::vector<int>> normalize_int_matrix_keep_inner(std::vector<std::vector<int>> values) {
...
static std::string int_matrix_to_string(const std::vector<std::vector<int>>& matrix) {
    ...
    return json::Value(rows).to_string();
        ...
        int main() {
            auto tc = load_cases("permutations");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto nums = cases[i]["input"][0].get<std::vector<int>>();
        auto actual = normalize_int_matrix_keep_inner(permute(nums));
