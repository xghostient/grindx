    ...
    return json::Value(rows).to_string();
        ...
        int main() {
            auto tc = load_cases("insert-interval");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto intervals = cases[i]["input"][0].get<std::vector<std::vector<int>>>();
        auto newInterval = cases[i]["input"][1].get<std::vector<int>>();
        auto actual = insert(intervals, newInterval);
        auto expected = cases[i]["expected"].get<std::vector<std::vector<int>>>();
        if (actual != expected) report_wa(i, cases[i]["input"].to_string(), to_string_matrix(expected), to_string_matrix(actual), total, cases[i]["category"].is_string() ? cases[i]["category"].get_string() : "");
