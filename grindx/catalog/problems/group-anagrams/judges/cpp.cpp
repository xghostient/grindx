...
static std::string string_matrix_to_string(const std::vector<std::vector<std::string>>& matrix) {
    ...
    return json::Value(rows).to_string();
        ...
        int main() {
            auto tc = load_cases("group-anagrams");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    auto strs = get_string_array(c["input"][0]);
    auto actual = normalize_string_groups(groupAnagrams(strs));
    auto expected = normalize_string_groups(get_string_matrix(c["expected"]));
    if (actual != expected) {
