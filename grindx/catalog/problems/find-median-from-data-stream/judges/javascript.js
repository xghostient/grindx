...
const Cls = result.MedianFinder;
if (typeof Cls !== "function") {
  process.stderr.write("Solution does not define a MedianFinder class\n");
  process.exit(2);
}
const tc = loadCases("find-median-from-data-stream");
...
for (let i = 0; i < cases.length; i++) {
  ...
  for (let j = 0; j < c.operations.length; j++) {
    ...
    let actual = null;
        if (op === "addNum") {
          obj.addNum(...args); actual = null;
        }
        if (op === "findMedian") {
          actual = obj.findMedian(...args);
        }
