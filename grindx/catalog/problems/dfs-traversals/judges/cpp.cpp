...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("dfs-traversals");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    TreeNode* root = list_to_tree(c["input"][0].get_array());
    json::Array actual;
    actual.push_back(json::Value(preorderTraversal(root)));
    actual.push_back(json::Value(inorderTraversal(root)));
    actual.push_back(json::Value(postorderTraversal(root)));
