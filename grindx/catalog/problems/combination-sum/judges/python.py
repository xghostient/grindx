"""Generated judge for combination-sum."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "combinationSum", None)
    if fn is None:
        print("Missing function: combinationSum", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("combination-sum")
    total = len(tc["cases"])
    for i, case in enumerate(tc["cases"]):
        ...
        if not compare(actual, case["expected"], "unordered_nested"):
