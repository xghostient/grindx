"""Generated judge for contains-duplicate."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "containsDuplicate", None)
    if fn is None:
        print("Missing function: containsDuplicate", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("contains-duplicate")
    total = len(tc["cases"])
