...
static bool is_balanced_tree(TreeNode* root) {
    ...
    return height(root).second;
        ...
        int main() {
            auto tc = load_cases("construct-binary-tree-from-preorder-and-inorder-traversal");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto& c = cases[i];
    auto preorder_vals = c["input"][0].get<std::vector<int>>();
    auto inorder_vals_input = c["input"][1].get<std::vector<int>>();
    json::Value actual(tree_to_list(buildTree(preorder_vals, inorder_vals_input)));
