...
static String intMatrixString(int[][] arr) {
    return Arrays.deepToString(arr);
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("construct-binary-tree-from-preorder-and-inorder-traversal");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    List<Object> input = (List<Object>) c.get("input");
    int[] preorderVals = Common.toIntArray((List<Object>) input.get(0));
    int[] inorderVals = Common.toIntArray((List<Object>) input.get(1));
    List<Object> actual = treeToList(new Solution().buildTree(preorderVals.clone(), inorderVals.clone()));
