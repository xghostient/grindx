...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("construct-binary-tree-from-preorder-and-inorder-traversal")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        preorder_vals = list(case["input"][0])
        inorder_vals_input = list(case["input"][1])
        actual = tree_to_list(mod.buildTree(preorder_vals, inorder_vals_input))
