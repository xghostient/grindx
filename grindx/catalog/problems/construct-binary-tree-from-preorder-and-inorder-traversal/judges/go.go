...
func isBalancedTree(root *TreeNode) bool {
	...
	return ok
        ...
        func main() {
        	tc := LoadCases("construct-binary-tree-from-preorder-and-inorder-traversal")
        	total := len(tc.Cases)
        for idx, c := range tc.Cases {
	var preorderVals []int
	var inorderVals []int
	var expected []interface{}
	json.Unmarshal(c.Input[0], &preorderVals)
	json.Unmarshal(c.Input[1], &inorderVals)
	json.Unmarshal(c.Expected, &expected)
	actual := TreeToList(buildTree(append([]int{}, preorderVals...), append([]int{}, inorderVals...)))
