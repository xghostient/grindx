...
func validateAlienOrder(words []string, order string) bool {
	...
	return true
        ...
        func main() {
	 tc := LoadCases("reconstruct-itinerary")
	 total := len(tc.Cases)
            for i, c := range tc.Cases {
    	var tickets [][]string
    	_ = json.Unmarshal(c.Input[0], &tickets)
    	actual := findItinerary(cloneStringMatrix(tickets))
    	var expected []string
    	_ = json.Unmarshal(c.Expected, &expected)
    	if !Compare(actual, expected, "exact") {
    		ReportWA(i, tickets, expected, actual, total, c.Category)
    	}
