...
static std::string int_matrix_to_string(const std::vector<std::vector<int>>& matrix) {
    ...
    return json::Value(rows).to_string();
        ...
        int main() {
            auto tc = load_cases("sort-colors");
        ...
        if (nums != expected) report_wa(i, cases[i]["input"].to_string(), json::Value(expected).to_string(), json::Value(nums).to_string(), total, cases[i]["category"].is_string() ? cases[i]["category"].get_string() : "");
