...
static std::string str_list_to_string(const std::vector<std::string>& items) {
    ...
    return json::Value(arr).to_string();
        ...
        int main() {
            auto tc = load_cases("letter-combinations-of-a-phone-number");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto digits = cases[i]["input"][0].get<std::string>();
        auto actual = normalize_str_list(letterCombinations(digits));
