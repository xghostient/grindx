  ...
  return true;
        ...
        const tc = loadCases("minimum-window-substring");
        const cases = tc.cases;
        const total = cases.length;

        for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  const actual = sol.minWindow(c.input[0], c.input[1]);
  if (!isValidMinWindow(c.input[0], c.input[1], c.expected, actual)) reportWa(i, c, actual, total);
