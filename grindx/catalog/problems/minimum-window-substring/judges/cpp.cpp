    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("minimum-window-substring");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        std::string s = cases[i]["input"][0].get<std::string>();
        std::string t = cases[i]["input"][1].get<std::string>();
        std::string actual = minWindow(s, t);
        std::string expected = cases[i]["expected"].get<std::string>();
        if (!is_valid_min_window(s, t, expected, actual)) report_wa(i, cases[i]["input"].to_string(), expected, actual, total, cases[i]["category"].is_string() ? cases[i]["category"].get_string() : "");
