...
return result
        ...
        func main() {
	tc := LoadCases("encode-and-decode-strings")
	total := len(tc.Cases)
        for i, c := range tc.Cases {
	var strs []string
	_ = json.Unmarshal(c.Input[0], &strs)
	encoded := encode(cloneStringSlice(strs))
	actual := decode(encoded)
	_ = encode(deriveProbeStrings(cloneStringSlice(strs)))
	roundtrip := decode(encoded)
	var expected []string
	_ = json.Unmarshal(c.Expected, &expected)
	if !Compare(actual, expected, "exact") || !Compare(roundtrip, expected, "exact") {
