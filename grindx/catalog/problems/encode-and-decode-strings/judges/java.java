...
static String[] deriveProbeStrings(String[] strs) {
    ...
    return out;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("encode-and-decode-strings");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    List<Object> input = (List<Object>) c.get("input");
    String[] strs = toStringArray((List<Object>) input.get(0));
    Solution sol = new Solution();
    String encoded = sol.encode(Arrays.copyOf(strs, strs.length));
    String[] actual = sol.decode(encoded);
    sol.encode(deriveProbeStrings(Arrays.copyOf(strs, strs.length)));
    String[] roundtrip = sol.decode(encoded);
    String[] expected = toStringArray((List<Object>) c.get("expected"));
    if (!Arrays.equals(actual, expected) || !Arrays.equals(roundtrip, expected)) {
