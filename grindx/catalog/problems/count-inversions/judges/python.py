"""Generated judge for count-inversions."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "countInversions", None)
    if fn is None:
        print("Missing function: countInversions", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("count-inversions")
    total = len(tc["cases"])
