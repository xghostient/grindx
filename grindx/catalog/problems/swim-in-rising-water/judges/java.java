...
static int[][][] toIntTensor3(List<Object> raw) {
    ...
    return out;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("swim-in-rising-water");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) { Map<String, Object> c = (Map<String, Object>) cases.get(i); List<Object> in = (List<Object>) c.get("input"); int[][] grid = Common.toIntMatrix((List<Object>) in.get(0)); int actual = new Solution().swimInWater(cloneMatrix(grid)); int expected = Common.toInt(c.get("expected")); if (actual != expected) Common.reportWA(i, in, expected, actual, total, (String) c.getOrDefault("category", ""));
