...
static std::string tasks_to_string(const std::vector<std::string>& tasks) {
    ...
    return out;
        ...
        int main() {
            auto tc = load_cases("k-closest-points-to-origin");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto points = cases[i]["input"][0].get<std::vector<std::vector<int>>>();
        int k = cases[i]["input"][1].get<int>();
        auto actual = normalize_int_matrix(kClosest(points, k));
