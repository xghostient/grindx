	...
	return result
        ...
        func main() {
	tc := LoadCases("num-ways-to-arrive-at-destination")
	total := len(tc.Cases)
	for i, c := range tc.Cases {
		var n int; _ = json.Unmarshal(c.Input[0], &n)
		var edges [][]int; _ = json.Unmarshal(c.Input[1], &edges)
		actual := countPaths(n, cloneMatrix(edges))
		var expected int; _ = json.Unmarshal(c.Expected, &expected)
		if actual != expected { ReportWA(i, []any{n, edges}, expected, actual, total, c.Category) }
