...
function validCourseOrder(order, numCourses, prerequisites) {
  ...
  return true;
        ...
        function run(solutionPath) {
          ...
          const tc = loadCases("course-schedule-ii");
          const cases = tc.cases;
          const total = cases.length;
        const fn = sol.findOrder;
for (let i = 0; i < cases.length; i++) {
  const c = cases[i];
  const actual = fn(c.input[0], cloneMatrix(c.input[1]));
  if (c.expected.length === 0) {
    if (!Array.isArray(actual) || actual.length !== 0) reportWa(i, c, actual, total);
  } else if (!validCourseOrder(actual, c.input[0], c.input[1])) {
