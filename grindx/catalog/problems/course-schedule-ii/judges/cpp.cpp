...
static bool valid_course_order(const std::vector<int>& order, int num_courses, const std::vector<std::vector<int>>& prerequisites) {
    ...
    return true;
        ...
        int main() {
            auto tc = load_cases("course-schedule-ii");
            auto& cases = tc["cases"].get_array();
            int total = static_cast<int>(cases.size());
            for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        int num_courses = c["input"][0].get<int>();
        std::vector<std::vector<int>> prerequisites = c["input"][1].get<std::vector<std::vector<int>>>();
        auto actual = findOrder(num_courses, prerequisites);
        auto expected = c["expected"].get<std::vector<int>>();
        if (expected.empty()) {
            if (!actual.empty()) {
                report_wa(i, c["input"].to_string(), grindx_vector_to_string(expected), grindx_vector_to_string(actual), total, c["category"].is_string() ? c["category"].get_string() : "");
            }
        } else if (!valid_course_order(actual, num_courses, prerequisites)) {
            report_wa(i, c["input"].to_string(), grindx_vector_to_string(expected), grindx_vector_to_string(actual), total, c["category"].is_string() ? c["category"].get_string() : "");
        }
