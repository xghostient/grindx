...
int main() {
    auto tc = load_cases("add-two-numbers");
    auto& cases = tc["cases"].get_array();
    int total = static_cast<int>(cases.size());
    for (int i = 0; i < total; i++) {
        auto& c = cases[i];
        ListNode* a = list_to_linked_list(c["input"][0].get_array());
        ListNode* b = list_to_linked_list(c["input"][1].get_array());
        json::Value actual(linked_list_to_list(addTwoNumbers(a, b)));
