...
class Judge {
    ...
    public static void main(String[] args) {
        Map<String, Object> tc = Common.loadCases("add-two-numbers");
        List<Object> cases = (List<Object>) tc.get("cases");
        int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
            Map<String, Object> c = (Map<String, Object>) cases.get(i);
            List<Object> input = (List<Object>) c.get("input");
            int[] arrA = Common.toIntArray((List<Object>) input.get(0));
            int[] arrB = Common.toIntArray((List<Object>) input.get(1));
            List<Integer> actual = linkedListToList(new Solution().addTwoNumbers(listToLinkedList(arrA), listToLinkedList(arrB)));
