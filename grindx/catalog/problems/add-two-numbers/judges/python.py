...
def run(solution_path: str) -> None:
    ...
    tc = load_cases("add-two-numbers")
    cases = tc["cases"]
    total = len(cases)
    for i, case in enumerate(cases):
        head_a = list_to_linked_list(list(case["input"][0]))
        head_b = list_to_linked_list(list(case["input"][1]))
        result = mod.addTwoNumbers(head_a, head_b)
