    ...
    int main() {
        auto tc = load_cases("assign-cookies");
        auto& cases = tc["cases"].get_array();
        int total = static_cast<int>(cases.size());
        for (int i = 0; i < total; i++) {
    auto arr1 = cases[i]["input"][0].get<std::vector<int>>();
    auto arr2 = cases[i]["input"][1].get<std::vector<int>>();
    int actual = findContentChildren(arr1, arr2);
