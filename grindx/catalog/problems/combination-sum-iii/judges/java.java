...
static List<List<Integer>> normalizeIntMatrix(int[][] raw) {
    ...
    return rows;
            ...
            public static void main(String[] args) {
                Map<String, Object> tc = Common.loadCases("combination-sum-iii");
                List<Object> cases = (List<Object>) tc.get("cases");
                int total = cases.size();
        for (int i = 0; i < cases.size(); i++) {
    Map<String, Object> c = (Map<String, Object>) cases.get(i);
    List<Object> input = (List<Object>) c.get("input");
    int k = Common.toInt(input.get(0));
    int n = Common.toInt(input.get(1));
    List<List<Integer>> actual = normalizeIntMatrix(new Solution().combinationSum3(k, n));
