"""Generated judge for longest-subarr-with-sum-k."""
...
def run(solution_path: str) -> None:
    ...
    fn = getattr(mod, "longestSubarrayWithSumK", None)
    if fn is None:
        print("Missing function: longestSubarrayWithSumK", file=sys.stderr)
        sys.exit(2)

    tc = load_cases("longest-subarr-with-sum-k")
    total = len(tc["cases"])
