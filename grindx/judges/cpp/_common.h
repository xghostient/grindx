...
struct Value {
    ...
    Value(long n) : data(static_cast<double>(n)) {}
    Value(long long n) : data(static_cast<double>(n)) {}
    ...
    int get_int() const { return static_cast<int>(std::get<double>(data)); }
    long long get_long() const { return static_cast<long long>(std::get<double>(data)); }
...
struct Node {
    ...
    Node() : val(0) {}
    Node(int x) : val(x) {}
    Node(int x, std::vector<Node*> n) : val(x), neighbors(n) {}
};
#endif
